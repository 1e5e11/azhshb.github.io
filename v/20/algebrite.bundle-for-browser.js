...
// 4 digits create two collisions
  ...
  */
  ...
  __factorial = function(n) {
    // Balanced products avoid multiplying a millions-digit accumulator
    // by one small integer on every iteration. Keep Algebrite's bigInt type.
    function product(lo, hi) {
      var result, i, mid;
      if (hi - lo < 32) {
        result = bigInt(1);
        for (i = lo; i <= hi; i++) result = mmul(result, bigInt(i));
        return result;
      }
      mid = Math.floor((lo + hi) / 2);
      return mmul(product(lo, mid), product(mid + 1, hi));
    }
    return product(2, n);
  };
